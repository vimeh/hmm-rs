...
pub fn execute_action(action: Action, app: &mut AppState) -> Result<()> {
    match action {
        ...
        Action::MoveCursorWordLeft => move_cursor_word_left(app),
        Action::MoveCursorWordRight => move_cursor_word_right(app),
        Action::DeleteWordBackward => delete_word_backward(app),
        Action::DeleteWordForward => delete_word_forward(app),
        Action::DeleteToEnd => delete_to_end(app),
        Action::DeleteToStart => delete_to_start(app),
        Action::PasteAtCursor => paste_at_cursor(app),
...
fn move_cursor_word_left(app: &mut AppState) {
    if let AppMode::Editing { buffer, cursor_pos } = &mut app.mode {
        if *cursor_pos == 0 {
            return;
        }
...
fn move_cursor_word_right(app: &mut AppState) {
    if let AppMode::Editing { buffer, cursor_pos } = &mut app.mode {
        let len = buffer.len();
        if *cursor_pos >= len {
            return;
...
fn delete_word_backward(app: &mut AppState) {
    if let AppMode::Editing { buffer, cursor_pos } = &mut app.mode {
        if *cursor_pos == 0 {
            return;
        }
...
fn delete_word_forward(app: &mut AppState) {
    if let AppMode::Editing { buffer, cursor_pos } = &mut app.mode {
        let len = buffer.len();
        if *cursor_pos >= len {
            return;
...
fn delete_to_end(app: &mut AppState) {
    if let AppMode::Editing { buffer, cursor_pos } = &mut app.mode {
        buffer.truncate(*cursor_pos);
    }
}

fn delete_to_start(app: &mut AppState) {
    if let AppMode::Editing { buffer, cursor_pos } = &mut app.mode {
...
fn paste_at_cursor(app: &mut AppState) {
    if let AppMode::Editing { buffer, cursor_pos } = &mut app.mode {
...
fn modify_stars(app: &mut AppState, change: i32) {
    if let Some(active_id) = app.active_node_id {
        ...
        node.title = node.title.replace(['★', '☆'], "").trim().to_string();
...
mod tests {
    ...
    fn test_paste_at_cursor() {
        ...
        // Note: We can't easily test actual clipboard integration in unit tests
        // because it requires system clipboard access.
        // The paste_at_cursor function would need to be refactored to accept
        // clipboard content as a parameter for better testability.

        // For now, we can test that the function doesn't panic when called
        paste_at_cursor(&mut app);

        // We can also test the text cleaning logic separately
        let test_content = "Line 1\nLine 2\tTabbed".to_string();
        let cleaned = test_content
            .replace('\n', " ")
            .replace('\r', "")
            .replace('\t', "  ");
        assert_eq!(cleaned, "Line 1 Line 2  Tabbed");
    }
